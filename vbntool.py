...
# check if file is quarantined using old format (one XOR encoding)
if (unpack("<Q", xor([chr(i) for i in vbn[qfm_offset+24:qfm_offset+32]], '0x5A'))[0] >= len(vbn)):
    logger.info("{} metadata is missing. File might be encoded using old quarantine format.".format(qfile_path))
    ...
    if args.output:
        ...
        logger.info("Writing decoded {} to {}".format(qfile_path, out_name))
